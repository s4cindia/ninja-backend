...
export const epubController = {
  ...
  async auditFromBuffer(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          `Failed to auto-create remediation plan for job ${job.id}`,
          remediationError instanceof Error ? remediationError : undefined,
        );
      }
  ...
  async auditFromFileId(req: AuthenticatedRequest, res: Response) {
    ...
    try {
      ...
      processAuditInBackground(job.id, fileRecord as FileRecord).catch(
        (error) => {
          logger.error(
            `Background audit failed for job ${job.id}: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        },
      );
    } catch (error) {
      logger.error(
        `EPUB audit from fileId failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
  ...
  async getAuditResult(req: Request, res: Response) {
    try {
      ...
      const combinedIssues = output?.combinedIssues as
  ...
  async createRemediationPlan(req: Request, res: Response) {
    ...
    try {
      ...
      logger.info(
        `  By Source: EPUBCheck=${plan.stats.bySource?.epubCheck || 0}, ACE=${plan.stats.bySource?.ace || 0}, JS Auditor=${plan.stats.bySource?.jsAuditor || 0}`,
      );
      logger.info(
        `  By Classification: Auto=${plan.stats.autoFixable}, QuickFix=${plan.stats.quickFixable}, Manual=${plan.stats.manualRequired}`,
      );

      logger.info("\nTASK TYPES IN RESPONSE:");
      plan.tasks.forEach((task, i) => {
        logger.info(
          `  ${i + 1}. [${task.type}] ${task.issueCode} @ ${task.location || "N/A"}`,
        );
      });
  ...
  async getRemediationSummary(req: Request, res: Response) {
    try {
      ...
      logger.error(
        "Failed to get remediation summary",
        error instanceof Error ? error : undefined,
      );
      return res.status(500).json({
        success: false,
  ...
  async updateTaskStatus(req: Request, res: Response) {
    try {
      ...
      const { status, resolution, resolvedBy, notes, completionMethod } =
        req.body;
  ...
  async markManualTaskFixed(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const task = await remediationService.markManualTaskFixed(jobId, taskId, {
        notes,
        verifiedBy,
        resolution,
      });
    ...
    try {
      ...
      if (!req.file) {
        return res.status(400).json({
          success: false,
          error:
            "No EPUB file uploaded. Please upload the remediated EPUB file.",
        });
      }

      logger.info(
        `[Re-audit] Starting re-audit for job ${jobId}, file: ${req.file.originalname}`,
      );
      ...
      logger.info(
        `[Re-audit] Completed: ${result.resolved} issues resolved, ${result.stillPending} still pending`,
      );
  ...
  async transferToAcr(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      logger.info(
        `[ACR Transfer] Job ${jobId}: ${result.transferredTasks} tasks transferred`,
      );
      ...
      return res.status(500).json({
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to transfer to ACR workflow",
      });
    }
  },
  ...
  async getAcrWorkflow(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const workflow = await remediationService.getAcrWorkflow(
        acrWorkflowId,
        tenantId,
      );
      ...
      logger.error(
        "Failed to get ACR workflow",
        error instanceof Error ? error : undefined,
      );
      return res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to get ACR workflow",
      });
    }
  },
  ...
  async updateAcrCriteria(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      if (
        !status ||
        !["verified", "failed", "not_applicable"].includes(status)
      ) {
        return res.status(400).json({
          success: false,
    ...
    try {
      ...
      if (!epubBuffer) {
        return res.status(400).json({
          success: false,
          error:
            "EPUB file not found. File may have been deleted or not uploaded.",
        });
      }
      ...
      return res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : "Auto-remediation failed",
      });
    }
  },
  ...
  async downloadRemediatedFile(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const output = job.output as {
        autoRemediation?: { remediatedFileName?: string };
      } | null;
      ...
      const fileBuffer = await fileStorageService.getRemediatedFile(
        jobId,
        remediatedFileName,
      );
      if (!fileBuffer) {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${remediatedFileName}"`,
      );
  ...
  async applySpecificFix(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const remediatedFileName = originalFileName.replace(
        /\.epub$/i,
        "_remediated.epub",
      );
      ...
      switch (fixCode) {
        case "EPUB-META-001":
          ...
          break;
        case "EPUB-META-002":
          results = await epubModifier.addAccessibilityMetadata(
            zip,
            options?.features,
          );
          break;
        case "EPUB-META-003":
          results = [
            await epubModifier.addAccessibilitySummary(zip, options?.summary),
          ];
          break;
        case "EPUB-SEM-001":
          results = await epubModifier.addHtmlLangAttributes(
            zip,
            options?.language,
          );
          break;
        ...
        case "EPUB-SEM-003":
          ...
          if (options?.changes && Array.isArray(options.changes)) {
            const epubTypesToFix = options.changes.map(
              (change: { epubType: string; role: string }) => ({
                epubType: change.epubType,
                role: change.role,
              }),
            );
            results = await epubModifier.addAriaRolesToEpubTypes(
              zip,
              epubTypesToFix,
            );
          } else {
            return res.status(400).json({
              success: false,
              error:
                "EPUB-SEM-003 requires options.changes array with epubType and role",
            });
          }
          break;
        case "EPUB-NAV-002":
  ...
  async getComparison(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const remediatedFileName = fileName.replace(
        /\.epub$/i,
        "_remediated.epub",
      );

      const originalBuffer = await fileStorageService.getFile(jobId, fileName);
      const remediatedBuffer = await fileStorageService.getRemediatedFile(
        jobId,
        remediatedFileName,
      );
  ...
  async startBatch(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const batch = await batchRemediationService.getBatchStatus(
        batchId,
        tenantId,
      );
      ...
      batchRemediationService
        .processBatch(batchId, tenantId, options || {})
  ...
  async cancelBatch(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const batch = await batchRemediationService.cancelBatch(
        batchId,
        tenantId,
      );
  ...
  async retryBatchJob(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const job = await batchRemediationService.retryJob(
        batchId,
        jobId,
        tenantId,
      );
  ...
  async listBatches(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      return res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to list batches",
      });
    }
  },
  ...
  async exportRemediated(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      logger.error(
        "Failed to export remediated EPUB",
        error instanceof Error ? error : undefined,
      );
      return res.status(500).json({
        success: false,
  ...
  async exportBatch(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const result = await epubExportService.exportBatch(
        jobIds,
        tenantId,
        options || {},
      );
  ...
  async getAccessibilityReport(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const report = await epubExportService.generateAccessibilityReport(
        jobId,
        tenantId,
      );
  ...
  async applyQuickFix(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      logger.debug(
        `applyQuickFix called: jobId=${jobId}, fixCode=${fixCode}, taskId=${taskId}, issueId=${issueId}`,
      );
      ...
      if (!epubBuffer) {
        return res
          .status(404)
          .json({ success: false, error: "EPUB file not found" });
      }
      ...
      if (
        ...
        fixCode === "EPUB-TYPE-HAS-MATCHING-ROLE"
      ) {
      ...
      if (changes && Array.isArray(changes)) {
        for (const change of changes) {
          ...
          const roleMatch =
            change.content?.match(/role="([^"]+)"/) ||
            change.newContent?.match(/role="([^"]+)"/);
      ...
      if (epubTypesToFix.length > 0) {
        logger.debug(
          `Using EPUB-TYPE case with regex method: ${epubTypesToFix.length} types to fix`,
        );
        ...
        logger.debug(
          `addAriaRolesToEpubTypes completed: ${results.length} modifications`,
        );
        ...
        try {
          ...
          if (plan?.tasks && Array.isArray(plan.tasks)) {
            const relatedTasks = plan.tasks.filter(
              (t: { status?: string; issueCode?: string; id?: string }) => {
            ...
            logger.debug(
              `Found ${relatedTasks.length} epub:type tasks to auto-complete`,
            );
            ...
            for (const task of relatedTasks) {
              try {
                ...
                tasksAutoCompleted++;
                logger.debug(
                  `Auto-completed task: ${task.id} (${task.issueCode})`,
                );
              } catch (err) {
                logger.error(
                  `Failed to auto-complete task ${task.id}`,
      ...
      if (changes && Array.isArray(changes) && changes.length > 0) {
        ...
        const result = await epubModifier.applyQuickFix(
          zip,
          changes,
          jobId,
          issueId,
        ...
        try {
          ...
          if (taskToUpdate) {
            ...
            const task = plan?.tasks.find(
              (t: { id?: string; issueId?: string }) =>
                t.id === taskToUpdate || t.issueId === taskToUpdate,
            );
            if (task) {
              await remediationService.updateTaskStatus(
                jobId,
  ...
  async scanEpubTypes(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      logger.debug(
        `scanEpubTypes called: jobId=${jobId}, tenantId=${tenantId}`,
      );
      ...
      logger.debug(
        `scanEpubTypes result: ${result.epubTypes.length} epub:types found`,
      );
...
// Interface for file record with nullable path (S3-stored files won't have local path)
...
async function processAuditInBackground(
  ...
  try {
    ...
    // Fetch file from S3 or local storage
    ...
    try {
      ...
      logger.warn(
        `Failed to auto-create remediation plan for job ${jobId}`,
        remediationError instanceof Error ? remediationError : undefined,
      );
    }
    ...
    logger.error(
      `Background audit failed for job ${jobId}`,
      error instanceof Error ? error : undefined,
    );
