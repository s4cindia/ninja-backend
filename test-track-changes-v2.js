/**
 * Test Track Changes Export - Excludes References section
 */
...
async function testTrackChanges() {
  ...
  console.log('Changed:', [...changeMap.entries()].map(([o, n]) => `${o}→${n}`).join(', '));
  console.log('Orphaned:', [...orphanedSet].join(', '));
  ...
  for (const pattern of refPatterns) {
    ...
    if (match && match.index !== undefined) {
      ...
      if (lastParagraphStart !== -1) {
        ...
        console.log(`Body length: ${bodyXML.length}, References length: ${referencesXML.length}`);
        break;
      }
    }
  }
  ...
  const outputPath = path.join(process.cwd(), 'test-track-changes-v2.docx');
  await fs.writeFile(outputPath, modifiedBuffer);

  console.log(`\n=== SAVED TO: ${outputPath} ===`);
  console.log('References section is EXCLUDED from changes!');
