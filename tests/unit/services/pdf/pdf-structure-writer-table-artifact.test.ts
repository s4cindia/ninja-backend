 ...
 * retags a confirmed-decorative /Table struct element as /Artifact, targeting
 * the specific table each issue's id refers to via findTargetTable -- the
 * same per-issue targeting fixSimpleTableHeaders already relies on, since a
 * whole-tree sweep would be wrong here (every other /Table on the page is an
 * unrelated finding, not an interchangeable duplicate the way LIST-IMPROPER-
 * MARKUP's orphaned LIs are).
 */
...
describe('PdfStructureWriterService.markTableAsArtifact', () => {
  it('retags only the table the issue is about, leaving other tables untouched', async () => {
    ...
    const tagOf = (ref: PDFRef): string | undefined => {
      const dict = doc.context.lookup(ref);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return (dict as any)?.get(PDFName.of('S'))?.toString();
    };

    // The table the issue named was retagged...
    expect(tagOf(tableOnPage2Ref)).toBe('/Artifact');
    // ...and the OTHER table (not named by any issue in this call) was not.
    expect(tagOf(tableOnPage1Ref)).toBe('/Table');
  });
  ...
  it('picks the right table by index among multiple tables on the same page', async () => {
    ...
    expect(tagOf(secondTableRef)).toBe('/Artifact');
    expect(tagOf(firstTableRef)).toBe('/Table');
  });

  /**
   * Regression for a real bug found live against Math_Kim (6 of 49 real
  ...
  it('fixes multiple tables on the same page in one batch without an earlier fix shifting a later lookup\'s index', async () => {
    ...
    expect(tagOf(firstTableRef)).toBe('/Artifact');
    expect(tagOf(secondTableRef)).toBe('/Artifact');
  });
