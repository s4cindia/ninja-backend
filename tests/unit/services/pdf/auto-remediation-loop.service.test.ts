...
describe('autoRemediationLoopService.startAutoLoop', () => {
  ...
  it('stops with reason "stalled" after consecutive rounds apply nothing, rather than exhausting the round ceiling', async () => {
    ...
    // it sitting at 'approved', so aiAnalysis.count never reaches 0 on its own.
  ...
  it('resets the stall counter once a round makes progress again', async () => {
    ...
    vi.mocked(prisma.aiAnalysis.updateMany).mockResolvedValue({ count: 1 } as any);
    vi.mocked(prisma.job.findUnique).mockResolvedValue({ id: 'job-1', tenantId: 'tenant-1', output: {} } as any);
    // Round 1 stalls (applied: 0), round 2 makes progress and converges --
    // a single stall must not carry over and trip the threshold later.
    ...
    // After round 1 (stalled, nothing applied) there's still outstanding
    // work; only after round 2 actually applies and re-audits does it clear.
    vi.mocked(prisma.aiAnalysis.count).mockResolvedValueOnce(5).mockResolvedValueOnce(0);
